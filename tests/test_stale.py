...
def test_model_can_flag_an_old_deal_as_stale():
    (deal, skipped), _ = run({"relevant": False, "stale": True})
    assert (deal, skipped) == (None, extract.STALE)
...
def test_unknown_or_unclear_dates_are_kept(event_date):
    ...
    (deal, skipped), _ = run(payload)
    assert deal is not None and skipped == ""
...
def test_irrelevant_article_is_neither_a_deal_nor_stale():
    (deal, skipped), _ = run({"relevant": False})
    assert (deal, skipped) == (None, "")
